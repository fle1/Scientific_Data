...
metabolites<-read.delim("../manuscript/Supplementary_table1.csv",sep = "\t",header = TRUE,comment.char = "#",stringsAsFactors = TRUE)
...
## 1.1 Normalize by sample fresh weight
...
### Series water
lines(x = c(0:6), y = raffinose_WT[c(7:13)],
      type = "c",pch= "_", col= "#00A600FF",lwd = 2)
...
write.table(x = data.frame(metadata,metabolites),file = "Processed_metabolite_table.txt",sep = "\t",col.names = TRUE,row.names = TRUE,quote = FALSE)
